...
impl fmt::Display for SyntaxError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Self::FunArgsStart(token) => write!(
                f,
                "expected the Start of a Functor Arguments List {}, but got {}",
                TokenKind::OpenParen,
                token
            ),
            Self::FunArgsEnd(token) => write!(
                f,
                "expected the End of a Functor Arguments List {}, but got {}",
                TokenKind::CloseParen,
                token
            ),
            Self::PrimaryStart(token) => write!(
                f,
                "expected the Start of a Primary Expression which is {} or {}, but got {}",
                TokenKind::OpenParen,
                TokenKind::Ident,
                token
            ),
            Self::PrimaryEnd(token) => write!(
                f,
                "expected the Start of a Primary Expression which is `)`, but got {} instead",
                token
            ),
        }
    }
