...
impl Expr {
    fn var_or_sym_from_name(name: &str) -> Expr {
        if name
            .chars()
            .next()
            .expect("Empty names are not allowed")
            .is_uppercase()
        {
    ...
    fn parse(lexer: &mut Peekable<impl Iterator<Item = Token>>) -> Result<Self, Error> {
        ...
        if let Some(_) = lexer.next_if(|t| t.kind == OpenParen) {
            ...
            if let Some(_) = lexer.next_if(|t| t.kind == CloseParen) {
                return Ok(Expr::Fun(
                    Box::new(Self::var_or_sym_from_name(&name.text)),
                    args,
                ));
            }
...
fn main() {
    ...
    if let Some(file_path) = args.next() {
        ...
        while !context.quit
            && lexer.peek().expect("Completely exhausted lexer").kind != TokenKind::End
        {
